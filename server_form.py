...
def init_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS ros (
            ...
            cv TEXT,
            mp TEXT,
            source_rnc_id INTEGER,
            FOREIGN KEY (user_id) REFERENCES users (id),
            FOREIGN KEY (assigned_user_id) REFERENCES users (id)
        )
    ''')

    # Migração: adicionar source_rnc_id na tabela ros (para bancos existentes)
    try:
        cursor.execute('ALTER TABLE ros ADD COLUMN source_rnc_id INTEGER')
    except sqlite3.OperationalError:
        pass
    ...
    try:
        ...
        cursor.execute('''
            SELECT 
                ...
                created_at, updated_at, source_rnc_id
            FROM ros
    ...
    try:
        ...
        cursor.execute('''
            INSERT INTO ros (
                ...
                price_note, area_responsavel, ass_responsavel,
                inspetor, responsavel, signature_inspection2_name,
                setor, purchase_order, position, cv, mp, source_rnc_id, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
        ''', (
            str(next_number),
...
@app.route('/api/ro/<int:ro_id>/convert-to-rnc', methods=['POST'])
def convert_ro_to_rnc(ro_id):
    """Reverte uma R.O de volta para RNC (desfaz a conversão)"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'error': 'Não autenticado'}), 401
    ...
    try:
        ...
        # Buscar R.O e verificar se veio de uma RNC
        cursor.execute('SELECT id, ro_number, source_rnc_id FROM ros WHERE id = ? AND (is_deleted = 0 OR is_deleted IS NULL)', (ro_id,))
        ro = cursor.fetchone()
        ...
        source_rnc_id = ro['source_rnc_id']
        if not source_rnc_id:
            return_db_connection(conn)
            return jsonify({'success': False, 'error': 'Este R.O não foi convertido de uma RNC'}), 400
        
        # Verificar se a RNC original ainda existe (soft-deleted)
        cursor.execute('SELECT id, rnc_number FROM rncs WHERE id = ?', (source_rnc_id,))
        rnc = cursor.fetchone()
        
        if not rnc:
            return_db_connection(conn)
            return jsonify({'success': False, 'error': 'RNC original não encontrada no banco de dados'}), 404
        
        # Restaurar a RNC (desfazer soft-delete)
        cursor.execute('UPDATE rncs SET is_deleted = 0, deleted_at = NULL WHERE id = ?', (source_rnc_id,))
        
        # Deletar a R.O permanentemente
        cursor.execute('DELETE FROM ros WHERE id = ?', (ro_id,))
        
        conn.commit()
        return_db_connection(conn)
        
        rnc_number = rnc['rnc_number'] if 'rnc_number' in rnc.keys() else source_rnc_id
        logger.info(f"R.O {ro['ro_number']} revertido para RNC {rnc_number} pelo usuário {user_id}")
        
        return jsonify({
            'success': True,
            'rnc_id': source_rnc_id,
            'rnc_number': str(rnc_number)
        })
    except Exception as e:
        logger.error(f"Erro ao converter R.O para RNC: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
